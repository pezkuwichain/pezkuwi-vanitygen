...
const { match, mnemonic, network, type, withCase } = yargs(hideBin(process.argv))
  ...
  .option('network', {
    choices: ['bizinikiwi', 'pezkuwi', 'dicle', 'zagros'],
    default: 'pezkuwi'
  })
  .option('type', {
...
switch (network) {
  case 'dicle':
    options.ss58Format = 2; // Dicle (formerly Kusama)
    break;

  case 'pezkuwi':
    options.ss58Format = 42; // Pezkuwichain main relay
    break;

  case 'zagros':
    options.ss58Format = 42; // Zagros canary network
    break;

  case 'bizinikiwi':
    options.ss58Format = 42; // Generic substrate format
    break;

  default:
    options.ss58Format = 42;
    break;
}
